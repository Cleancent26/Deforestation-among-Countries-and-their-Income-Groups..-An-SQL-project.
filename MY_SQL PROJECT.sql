...
SELECT * FROM REGION
;
			--START OF DATA_CLEANING--
/*EFFECTING CHANGES TO COUNTRY NAME FROM 'BAHAMAS, THE' TO 'THE BAHAMAS'  
			AND'GAMBIA, THE' TO 'THE GAMBIA'
*/
...
/*DISTINCT NAMES OF ALL COUNTRIES INVOLVED*/
SELECT distinct country_name FROM FOREST_AREA
ORDER BY COUNTRY_NAME ASC;


/*COUNTRIES WITH  NULL VALUES IN THE FOREST_AREA_SQKM COLUMN
FOR THE ENTIRE SCOPE(1989-2016)  USING SUBQUERY */
...
/*DELETED COUNTRIES WITH  NULL VALUES IN THE FOREST_AREA_SQKM COLUMN
FOR THE ENTIRE SCOPE(1989-2016)  USING SUBQUERY */
...
/*DELETED COUNTRIES WITH ONLY 6 OUT OF THE 27 forest_area_sqkm THAT IS NOT NULL
...
DELETE FROM Forest_Area WHERE COUNTRY_NAME =
 (SELECT DISTINCT country_name FROM Forest_Area
  ...
  --ASSESSMENT OF 'ETHIOPIA'
SELECT * FROM FOREST_AREA
WHERE country_name = 'ETHIOPIA' AND  forest_area_sqkm  IS NOT NULL
ORDER  BY YEAR ASC;

--CALCULATED AVG OF forest_area_sqkm(MEAN) FOR ETHIOPIA 
SELECT AVG(forest_area_sqkm) T FROM FOREST_AREA
WHERE country_name = 'ETHIOPIA' ;

/*UPDATED forest_area_sqkm FOR ETHIOPIA BETWEEN THE YEAR 1990-1992
WHERE forest_area_sqkm IS NULL WITH THE AVERAGE VALUE(MEAN) OF forest_area_sqkm */
UPDATE Forest_Area 
...
/* CONVERTED AND UPDATED THE INSERTED FLOAT AVERAGE VALUE TO INTEGER
FOR ETHIOPIA BETWEEN YEAR 1990-1992 */
...
WHERE country_name = 'ETHIOPIA' AND YEAR BETWEEN 1990 AND 1992
;
--ASSESSMENT(CROSS_CHECKING) OF FOREST_AREA TABLE
SELECT COUNTRY_NAME,YEAR,forest_area_sqkm FROM FOREST_AREA
WHERE   forest_area_sqkm IS NULL;

----ASSESSMENT(CROSS_CHECKING) OF FOREST_AREA TABLE FOR COUNTRY 'St. Martin (French part)'
WITH HIGHEST_FREQUENCY AS (SELECT YEAR,forest_area_sqkm, DENSE_RANK () 
OVER (PARTITION BY forest_area_sqkm ORDER BY YEAR DESC) MODE_VALUE FROM Forest_Area T
WHERE country_name = 'St. Martin (French part)' )
SELECT forest_area_sqkm,MAX(MODE_VALUE) J FROM HIGHEST_FREQUENCY
GROUP BY forest_area_sqkm
ORDER BY J DESC
;
/*USING THE MOST OCCURING forest_area_sqkm (MODE) 
 TO FILL IN THE VALUE FOR St. Martin (French part) FOR YEAR 2016 */
UPDATE Forest_Area 
...
/*USING SUBQUERY TO GET DISTINCT COUNTRY_NAME AND THEIR HIGHEST COUNT(MODE_RANK)
OF NULL VALUE FOR TOTAL_AREA_SQ_MI FROM PREVIOUS PARTITION */
...
/*DELETED COUNTRIES WITH  NULL VALUES IN THE LAND_AREA_SQKM COLUMN
FOR THE ENTIRE SCOPE(1989-2016) i.e SUDAN AND SOUTH SUDAN USING SUBQUERY */
...
/*UPDATED NULL VALUE FOR Northern Mariana Islands IN total_area_sq_mi 
FOR YEAR 1990 USING MODE*/
 UPDATE Land_Area 
 SET total_area_sq_mi = 
(SELECT total_area_sq_mi FROM 
(SELECT total_area_sq_mi,COUNT(total_area_sq_mi) AS MODE_VALUE FROM LAND_AREA
...
NOTE;
 YOU CAN INCLUDE MORE ANALYTICAL QUESTIONS TO COVER MORE INSIGHTS.

*/

-- Q1.     WHAT ARE THE TOTAL NUMBER OF COUNTRIES INVOLVED IN DEFORETATION?
SELECT *, 
RANK () OVER (PARTITION BY FOREST_AREA_SQKM ORDER BY YEAR)
FROM FOREST_AREA;
...
/* Q2.   SHOW THE INCOME GROUPS OF COUNTRIES HAVING TOTAL AREA RANGING FROM
				75,000 TO 150,000 SQUARE METER
*/
SELECT DISTINCT L.COUNTRY_NAME,L.TOTAL_AREA_SQ_MI, R.INCOME_GROUP  
FROM REGION R LEFT JOIN LAND_AREA L ON R.COUNTRY_CODE = L.COUNTRY_CODE
WHERE TOTAL_AREA_SQ_MI BETWEEN  75000 AND 150000
 ; 
/*Q3.  CALCULATE AVG. AREA IN SQUARE MILES FOR COUNTRIES IN THE	'UPPER MIDDLE 
		INCOME REGION'	COMPARE THE RESULT WITH THE REST OF THE CATEGORIES
*/
SELECT R.COUNTRY_NAME,AVG(L.TOTAL_AREA_SQ_MI) AVG_OF_TOTAL_AREA_SQ_MI,R.INCOME_GROUP  
FROM REGION R LEFT JOIN LAND_AREA L ON R.COUNTRY_CODE = L.COUNTRY_CODE
WHERE INCOME_GROUP =  'UPPER MIDDLE INCOME'
GROUP BY R.COUNTRY_NAME,R.INCOME_GROUP
ORDER BY AVG(L.TOTAL_AREA_SQ_MI) DESC;

-- OTHERS FOR LOWER MIDDLE INCOME
...
/*4.DETERMINE THE TOTAL FOREST AREA IN SQUARE KM FOR COUNTRIES IN THE 'HIGH INCOME' 
	GROUP COMPARE RESULT WITH THE REST OF THE INCOME CATEGORIES
	*/
SELECT R.COUNTRY_NAME,SUM(F.FOREST_AREA_SQKM) TOTAL_FOREST_AREA_SQKM,R.INCOME_GROUP  
FROM REGION R LEFT JOIN FOREST_AREA F ON R.COUNTRY_CODE = F.COUNTRY_CODE
...
--Q5.SHOW COUNTRIES FROM EACH REGION (CONTINENT) HAVING THE HIGHEST TOTAL FOREST AREAS.
